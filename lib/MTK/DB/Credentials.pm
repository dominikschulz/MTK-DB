...
sub BUILD {
  ...
  } ## end else [ if ( $self->_detect_credentials...)]
...
sub _detect_credentials {
  ...
  foreach my $key (@keys) {
    ...
    foreach my $password (@passwords) {
      ...
      $database ||= 'mysql';
      $self->logger()->log(
        message => 'Adding hard-coded key for checking (MTK::MySQL::User::' . $key . q{ -> } . $username . q{:} . $password . q{/} . $database . q{)},
        level   => 'debug',
      );
      my $arg_ref = {
        'username' => $username,
      ...
      $arg_ref->{'socket'} = $socket if ( $socket && -e $socket );
      push( @{ $self->candidates() }, $arg_ref, );
    } ## end foreach my $password (@passwords)
  ...
  if ( my ( $username, $password, $socket ) = $self->_read_mycnf($debiancnf) ) {
    $self->logger()
      ->log( message => 'Adding auto-detected credentials for checking from /etc/debian.cnf (' . $username . q{:} . $password . q{)}, level => 'debug', );
    my $arg_ref = {
      'username' => $username,
      'password' => $password,
      'database' => 'mysql',
  ...
  foreach my $try ( 1 .. $self->num_tries() ) {
    foreach my $creds ( sort_root_up( @{ $self->candidates() } ) ) {
      ...
      if ( $self->_test_connection( $username, $password, $database, $creds->{'socket'} ) ) {
        ...
        $self->logger()
          ->log( message => 'Search finished. Found valid credentials (' . $username . q{:} . $password . q{/} . $database . q{)}, level => 'debug', );
        return 1;
      } ## end if ( $self->_test_connection...)
      else {
        $self->logger()->log( message => 'Continuing search ...', level => 'debug', );
      }
    } ## end foreach my $creds ( sort_root_up...)

    if ( $try < $self->num_tries() ) {
      sleep $self->sleep();
    }
  } ## end foreach my $try ( 1 .. $self...)

  # nothing found ...
  $self->logger()->log( message => 'Search exhausted. No valid credentials found.', level => 'debug', );
  return;
} ## end sub _detect_credentials
